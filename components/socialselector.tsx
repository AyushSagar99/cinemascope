...
export default function SocialContextSelector() {
  ...
  const contexts: { value: SocialContext; label: string; }[] = [
    { 
      value: 'Solo Night', 
      label: 'Solo Night', 
    },
  ...
  const handleSelect = (context: SocialContext) => {
    dispatch({ type: 'SET_SOCIAL_CONTEXT', payload: context });
    setIsOpen(false);
  };
    ...
    <div className="w-full">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        <button
          ...
          type="button"
        >
          <span className={state.selectedSocialContext ? 'text-gray-700' : 'text-gray-400'}>
            {state.selectedSocialContext || 'Select viewing context'}
          </span>
          <ChevronDown
                ...
                <li key={context.value}>
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-gray-700 font-medium hover:bg-gray-50 transition-colors duration-150 ${
                      state.selectedSocialContext === context.value 
